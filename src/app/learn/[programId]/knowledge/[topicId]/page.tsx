...
export default async function KnowledgeTopicPage({
  ...
  params: Promise<{ programId: string; topicId: string }>;
}) {
  ...
  const category = knowledge.knowledgeCategories.find(
    (c) => c.id === topic.categoryId,
  );
  const siblings = knowledge
  ...
  return (
    <>
      <KnowledgeReadTracker programId={programId} topicId={topic.id} />
      <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-8">
        <nav className="text-sm text-slate-500">
          ...
          <Link href={`${base}/knowledge`} className="hover:text-sky-600">
            知識庫
          </Link>
          {category && (
            <>
